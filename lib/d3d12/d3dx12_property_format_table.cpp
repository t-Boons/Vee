...
const LPCSTR D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::s_FormatNames[] =  // separate from above structure so it can be compiled out
                                                                    // of the runtime.
    {
        //   Name
        "UNKNOWN",
...
constexpr D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::FORMAT_DETAIL D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::s_FormatDetail[] = {
    ...
    //   BitsPerComponent[4], BitsPerUnit,    SRGB,  WidthAlignment, HeightAlignment, DepthAlignment,   Layout, TypeLevel,
    //   ComponentName[4],ComponentInterpretation[4],                          bDX9VertexOrIndexFormat bDX9TextureFormat,
    //   bFloatNormFormat, bPlanar, bYUV    bDependantFormatCastSet bInternal
    {
        DXGI_FORMAT_UNKNOWN,
        DXGI_FORMAT_UNKNOWN,
        D3DFCS_UNKNOWN,
...
//---------------------------------------------------------------------------------------------------------------------------------
// GetNumFormats
UINT D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::GetNumFormats() { return s_NumFormats; }
...
//---------------------------------------------------------------------------------------------------------------------------------
// GetFormatTable
BOOL D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::Opaque(DXGI_FORMAT Format) { return Format == DXGI_FORMAT_420_OPAQUE; }
...
HRESULT D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::CalculateExtraPlanarRows(DXGI_FORMAT format,
                                                                     UINT plane0Height,
                                                                     _Out_ UINT& totalHeight)
{
    ...
    // blockWidth, blockHeight, and blockSize only reflect the size of plane 0.  Each planar format has additonal planes that
    // must be counted.  Each format increases size by another 0.5x, 1x, or 2x.  Grab the number of "half allocation" increments
    // so integer math can be used to calculate the extra size.
                                                                  ...
                                                                  _Out_writes_opt_(subresources) D3D12_MEMCPY_DEST* pDst)
{
    ...
    if (tableIndex == UINT(-1)) return E_INVALIDARG;
    ...
    for (UINT s = 0, iM = 0; s < subresources; ++s)
    {
        ...
        {
            // The height must *not* be aligned to HeightAlign.  As there is no plane pitch/stride, the expectation is that the
            // 2nd plane begins immediately after the first.  The only formats with HeightAlignment other than 1 are planar or
            // block compressed, and block compressed is handled above.
            assert(formatDetail.bPlanar || formatDetail.HeightAlignment == 1);
            blockHeight = subHeight;

            // Combined with the division os subWidth by the width alignment above, this helps achieve rounding the stride up to
            // an even multiple of block width.  This is especially important for formats like NV12 and P208 whose chroma plane
            // is wider than the luma.
        ...
        static_assert((MAP_ALIGN_REQUIREMENT & (MAP_ALIGN_REQUIREMENT - 1)) == 0,
...
inline bool IsPow2(UINT Val) { return 0 == (Val & (Val - 1)); }
...
// also use the CalculateExtraPlanarRows function to calculate the corresonding height or use the
// CalculateMinimumRowMajorSlicePitch function. For Block Compressed Formats, this function returns the RowPitch of a row of
// blocks. For packed subsampled formats and other formats, this function returns the row pitch of one single row of pixels.
HRESULT D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::CalculateMinimumRowMajorRowPitch(DXGI_FORMAT Format,
                                                                             UINT Width,
                                                                             _Out_ UINT& RowPitch)
{
...
// planes.
HRESULT D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::CalculateMinimumRowMajorSlicePitch(DXGI_FORMAT Format,
                                                                               UINT TightRowPitch,
                                                                               UINT Height,
...
D3D_FORMAT_COMPONENT_INTERPRETATION D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::GetFormatComponentInterpretation(
    DXGI_FORMAT Format,
...
//---------------------------------------------------------------------------------------------------------------------------------
// Non-opaque Planar
BOOL D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::NonOpaquePlanar(DXGI_FORMAT Format) { return Planar(Format) && !Opaque(Format); }
...
void D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::GetYCbCrChromaSubsampling(DXGI_FORMAT Format,
...
void D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::GetTileShape(D3D12_TILE_SHAPE* pTileShape,
                                                      DXGI_FORMAT Format,
    ...
    switch (Dimension)
    {
        ...
        case D3D12_RESOURCE_DIMENSION_TEXTURE1D:
        {
            ...
            pTileShape->WidthInTexels =
                (BPU == 0) ? D3D12_TILED_RESOURCE_TILE_SIZE_IN_BYTES : D3D12_TILED_RESOURCE_TILE_SIZE_IN_BYTES * 8 / BPU;
        ...
        case D3D12_RESOURCE_DIMENSION_TEXTURE2D:
        {
            if (IsBlockCompressFormat(Format))
            {
                ...
                // These equations calculate the size in texels for a tile. It relies on the fact that 64 * 64 blocks fit in a
                // tile if the block size is 128 bits.
        ...
        case D3D12_RESOURCE_DIMENSION_TEXTURE3D:
        {
            ...
            {
                // Currently only supported block sizes are 64 and 128.
                // These equations calculate the size in texels for a tile. It relies on the fact that 16*16*16 blocks fit in a
                // tile if the block size is 128 bits.
                assert(BPU == 64 || BPU == 128);
...
void D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::Get4KTileShape(D3D12_TILE_SHAPE* pTileShape,
                                                        DXGI_FORMAT Format,
                                                        D3D12_RESOURCE_DIMENSION Dimension,
                                                        UINT SampleCount)
{
...
// Maps resource format + view format to a plane index for resource formats where the plane index can be inferred from this
// information. For planar formats where the plane index is ambiguous given this information (examples: V208, V408), this
// function returns 0. This function returns 0 for non-planar formats.
...
//---------------------------------------------------------------------------------------------------------------------------------
void D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::GetMipDimensions(UINT8 mipSlice,
                                                          _Inout_ UINT64* pWidth,
                                                          _Inout_opt_ UINT64* pHeight,
                                                          _Inout_opt_ UINT64* pDepth)
{
...
// ValidCastToR32UAV
//
// D3D11 has a limitation on typed UAVs (e.g. Texture1D/2D/3D) whereby the only format that can be read is R32_*.  Lots of
// formats can be written though, with type conversion (e.g. R8G8B8A8_*).  If an API user wants to do image processing in-place,
// in either the Compute Shader or the Pixel Shader, the only format available is R32_* (since it can be read and written at the
// same time).
//
// We were able to allow resources (Texture1D/2D/3D), created with a format from a small set of families that have 32 bits per
// element (such as R8G8B8A8_TYPELESS), to be cast to R32_* when creating a UAV.  This means the Compute Shader or Pixel Shader
// can do simultaneous read+write on the resource when bound as an R32_* UAV, with the caveat that the shader code has to do
// manual type conversion manually, but later on the resource can be used as an SRV or RT as the desired type (e.g.
// R8G8B8A8_UNORM), and thus have access to filtering/blending where the hardware knows what the format is.
//
...
// and do a rendering pass that copies from the R32_* UAV to whatever typed resource they really wanted.  For formats not
// included in this list, such as any format that doesn't have 32-bits per component, as well as some 32-bit per component
// formats like R24G8 or R11G11B10_FLOAT there is no alternative for an application but to do the extra copy as mentioned, or
// avoid in-place image editing in favor of ping-ponging between buffers with multiple passes.
//
bool D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::ValidCastToR32UAV(DXGI_FORMAT from, DXGI_FORMAT to)
{
    ...
    if (((to == DXGI_FORMAT_R32_UINT) || (to == DXGI_FORMAT_R32_SINT) || (to == DXGI_FORMAT_R32_FLOAT)) &&
        ((from == DXGI_FORMAT_R10G10B10A2_TYPELESS) || (from == DXGI_FORMAT_R8G8B8A8_TYPELESS) ||
         (from == DXGI_FORMAT_B8G8R8A8_TYPELESS) || (from == DXGI_FORMAT_B8G8R8X8_TYPELESS) ||
         (from == DXGI_FORMAT_R16G16_TYPELESS) || (from == DXGI_FORMAT_R32_TYPELESS)))
    {
        return true;
    }
    return false;
